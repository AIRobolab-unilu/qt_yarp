...
macro(checkandset_dependency package)
    if (${package}_FOUND)
        set(CUDDIE_HAS_${package} TRUE CACHE BOOL "Package ${package} found" FORCE)
        set(CUDDIE_USE_${package} TRUE CACHE BOOL "Use package ${package}")
        message(STATUS "${package}: found")
    else (${package}_FOUND)
        set(CUDDIE_HAS_${package} FALSE CACHE BOOL "" FORCE)
        set(CUDDIE_USE_${package} FALSE CACHE BOOL "Use package ${package}")
        message(STATUS "${package}: NOT found")
    endif (${package}_FOUND)
    mark_as_advanced(CUDDIE_HAS_${package})

    if (NOT ${package}_FOUND AND CUDDIE_USE_${package})
        message(WARNING "You requested to use the package ${package}, but it is unavailable (or was not found). This might lead to compile errors, we recommend you turn off the CUDDIE_USE_${package} flag.")
    endif (NOT ${package}_FOUND AND CUDDIE_USE_${package})

    #store all dependency flags for later export
    set_property(GLOBAL APPEND PROPERTY CUDDIE_DEPENDENCIES_FLAGS CUDDIE_USE_${package})
...
if (OpenCV_FOUND)
  ...
  if (OpenCV_VERSION_MAJOR EQUAL 2)
    ...
    set(CUDDIE_OpenCV_LEGACY false CACHE BOOL "Legacy version of OpenCV detected" FORCE)
    mark_as_advanced(CUDDIE_OpenCV_LEGACY)
  else()
    set(CUDDIE_OpenCV_LEGACY true CACHE BOOL "Legacy version of OpenCV detected" FORCE)
    message(STATUS "OpenCV is previous 2.0 (some modules will be skipped)")  
    message(STATUS "Setting CUDDIE_OpenCV_LEGACY true")
  endif()

  set(CUDDIE_LINK_DIRECTORIES ${CUDDIE_LINK_DIRECTORIES} ${OpenCV_LIB_DIR})
endif()
...
#                set(CUDDIE_GTK2_LEGACY true CACHE BOOL "Legacy version of GTK2 detected" FORCE)
#        else()
#                set(CUDDIE_GTK2_LEGACY false CACHE BOOL "Legacy version of GTK2 detected" FORCE)
#                mark_as_advanced(CUDDIE_GTK2_LEGACY)
#        endif()
#     endif()
#    endif()
#
#    # check version of GtkMM
#    if (NOT CUDDIE_GTK2_LEGACY)
#        message(STATUS "GTK2 is at least 2.12")
#   else()
#        message(STATUS "GTK2 is prior to 2.12 (some modules will be skipped)")  
#        message(STATUS "Setting CUDDIE_GTK2_LEGACY true")
#   endif()
#
#endif()
