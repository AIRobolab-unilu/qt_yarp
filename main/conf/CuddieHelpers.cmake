...
#
# -append ${target} to the list of targetes built within the project (global property CUDDIE_TARGETS)
...
# -append export rules for target to a CUDDIE_EXPORTBUILD_FILE in ${PROJECT_BINARY_DIR}
#
...
MACRO(cuddie_export_library target)
  ...
  set(CUDDIE_EXPORTBUILD_FILE cuddie-export-build.cmake)

  ##### Append target to global list.
  cuddie_set_property(GLOBAL APPEND PROPERTY CUDDIE_TARGETS ${target})
  # Install/export rules
  install(TARGETS ${target}
          EXPORT cuddie-targets
  ...
  export(TARGETS ${target} APPEND FILE ${CMAKE_BINARY_DIR}/${CUDDIE_EXPORTBUILD_FILE})
  ...
  # For CMAKE_VERSION >= 2.8.10 this is handled in CuddieOptions.cmake
...
ENDMACRO(cuddie_export_library)


macro(cuddie_export_plugin target)
    ...
    if("${_type}" STREQUAL "MODULE_LIBRARY")
        yarp_install(TARGETS ${target}
                     ...
                     ARCHIVE DESTINATION ${CUDDIE_STATIC_PLUGINS_INSTALL_DIR})
    else()
        cuddie_export_library(${target})
    endif()
endmacro()
...
# See cuddie_export_library2 for usage.
...
# make or the vs gui.
macro(cuddie_app target)
    set(dummy ${CMAKE_BINARY_DIR}/f-${target})
	file(TO_CMAKE_PATH "${CUDDIE_APPLICATIONS_PREFIX}" app_prefix)

	set(cmakefile ${CMAKE_BINARY_DIR}/app-${target}.cmake)
    
    set(BUFFER "###### This file is automatically generated by CMake.\n")

    set(BUFFER "${BUFFER}## see macro cuddie_app defined in conf/CuddieHelpers.cmake\n\n")
    ...
    cuddie_set_property(GLOBAL APPEND PROPERTY CUDDIE_APPLICATIONS app-${target})
    cuddie_set_property(GLOBAL APPEND PROPERTY CUDDIE_APPLICATIONS_FILES ${dummy})
...
#
# cuddie_app_install(target
#                  [DESTINATION dest]
#                  [VERBOSE]
#                  [ROOT]
#                  [FILES file1 file2 ...])
# - target: name of the command, previously defined with the cuddie_app command
# - DESTINATION: destination directory for files
# - VERBOSE: print verbose
# - ROOT: whether to use app/${target} as prefix or not
# - FILES: files to be copied
#
# By default FILES will be copied to ${CUDDIE_INSTALL_PREFIX}/app/${target}/DESTINATION
# if you specify ROOT, files will be copied to ${CUDDIE_INSTALL_PREFIX}.
#
...
function(cuddie_app_install target)
  PARSE_ARGUMENTS(${target}
	"FILES;DESTINATION"
  ...
  if (root)
	file(TO_CMAKE_PATH "${CUDDIE_APPLICATIONS_PREFIX}" dapp)
  else(root)
	file(TO_CMAKE_PATH "${CUDDIE_APPLICATIONS_PREFIX}/app/${target}" dapp)
  endif(root)
...
endfunction(cuddie_app_install)

##
# This function is now obsolete.
# Leaving it empty for backward compatibility.
macro(cuddie_add_target target)
## obsolete...
endmacro(cuddie_add_target)

## gather all applications from global list CUDDIE_APPLICATIONS
# declare a target called install_applications which depends
# on all the previously defined applications.
macro(cuddie_app_all)
    ## add custom targets
    cuddie_get_property(applications GLOBAL PROPERTY CUDDIE_APPLICATIONS)
...
endmacro(cuddie_app_all)

### From yarp.
# Helper macro to work around a bug in set_property in cmake 2.6.0
# We use cuddie_ prefix to avoid name clashes with yarp.
MACRO(cuddie_get_property localname _global _property varname)
  set(_cuddie_exists_chk)
  get_property(_cuddie_exists_chk GLOBAL PROPERTY ${varname})
  if (_cuddie_exists_chk)
    set(${localname} ${_cuddie_exists_chk})
  else (_cuddie_exists_chk)
    set(${localname})
  endif (_cuddie_exists_chk)
ENDMACRO(cuddie_get_property)

# Helper macro to work around a bug in set_property in cmake 2.6.0
MACRO(cuddie_set_property _global _property _append varname)
  get_property(_cuddie_append_chk GLOBAL PROPERTY ${varname})
  if (_cuddie_append_chk)
    set_property(GLOBAL APPEND PROPERTY ${varname} ${ARGN})
  else (_cuddie_append_chk)
    set_property(GLOBAL PROPERTY ${varname} ${ARGN})
  endif (_cuddie_append_chk)
ENDMACRO(cuddie_set_property)
