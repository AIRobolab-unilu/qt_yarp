...
MACRO(cuddie_install_with_rpath)
    #########################################################################
    # Control setting an rpath
    if (NOT MSVC)
        option(CUDDIE_INSTALL_WITH_RPATH "Set an rpath after installing the executables" FALSE)
        #mark_as_advanced(CUDDIE_ENABLE_FORCE_RPATH)
    endif (NOT MSVC)

    # By default do not build with rpath.
    # If this flag is true then all the variables related to RPATH are ignored
    if(CUDDIE_INSTALL_WITH_RPATH)
        ...
        if(CMAKE_VERSION VERSION_LESS 2.8.12)
            ...
            set(CUDDIE_FULL_BIN_DIR "${CMAKE_INSTALL_PREFIX}/bin")
            set(CUDDIE_FULL_LIB_DIR "${CMAKE_INSTALL_PREFIX}/lib")

            file(RELATIVE_PATH _rel_path "${CUDDIE_FULL_BIN_DIR}" "${CUDDIE_FULL_LIB_DIR}")
    ...
    endif(CUDDIE_INSTALL_WITH_RPATH)

ENDMACRO(cuddie_install_with_rpath)
...
if(NOT CMAKE_CONFIGURATION_TYPES)
    ...
    set(CUDDIE_BUILD_TYPES "Debug" "Release" "MinSizeRel" "RelWithDebInfo")
    set_property(CACHE CMAKE_BUILD_TYPE PROPERTY STRINGS ${CUDDIE_BUILD_TYPES})
endif()

########################################################################
# settings for rpath

cuddie_install_with_rpath() #from cuddieHelpers
...
if(NOT MSVC OR NOT CMAKE_VERSION VERSION_LESS 3.4)
    option(CUDDIE_SHARED_LIBRARY "Compile shared libraries rather than static libraries" FALSE)
...
# CuddieHelpers.cmake, in the cuddie_export_library macro (and obviously
