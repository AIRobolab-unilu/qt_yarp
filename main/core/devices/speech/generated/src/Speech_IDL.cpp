...
void Speech_IDL_play::init(const std::string& filename) {
  _return = false;
...
bool Speech_IDL::play(const std::string& filename) {
  bool _return = false;
  Speech_IDL_play helper;
  helper.init(filename);
  if (!yarp().canWrite()) {
    yError("Missing server method '%s'?","bool Speech_IDL::play(const std::string& filename)");
  }
  bool ok = yarp().write(helper,helper);
  return ok?helper._return:_return;
}
...
std::vector<std::string> Speech_IDL::help(const std::string& functionName) {
  ...
  if(showAll) {
    ...
    if (functionName=="play") {
      helpString.push_back("bool play(const std::string& filename) ");
      helpString.push_back("play the previously rendered speech file (wave) ");
    }
